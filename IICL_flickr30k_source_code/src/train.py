...
def train(args):
    ...
    if (args.recipe_loss_weight > 0 and use_extra_data) or (args.intraCL_loss_weight > 0 and use_extra_data) :       # 1.0 and True
        ...
        loader_textonly, _ = get_loader(args.root, args.batch_size*2,     # DATASET_PATH, 128*2
                                        args.resize,     # 256
                                        args.imsize,     # 224
                                        augment=True,
    ...
    vocab_size = len(dataset.get_vocab())          #16303
    model = get_model(args, vocab_size)
    
   
    params_backbone = list(model.image_encoder.backbone.parameters())     #获取预训练模型参数
    ...
    optimizer = get_optimizer(params_fc,
                              params_backbone,
                              args.lr, args.scale_lr, args.wd,    #0.0001   1.0   0.0
                              freeze_backbone=args.freeze_backbone)    #False
